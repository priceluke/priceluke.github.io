    ...
    Sub trend_array(ByVal full_array(,) As String, ByVal timescale As String, ByVal id As Integer, ByVal productorcomponent As String)
        ...
        For i = 0 To (full_array.Length / 3) - 1
            If full_array(i, 0) = id Then
                ...
                Dim expenddt As Date = Date.ParseExact(initial_date, "dd/MM/yyyy hh:mm:ss", System.Globalization.DateTimeFormatInfo.InvariantInfo)
        ...
        Select Case timescale
            Case "Hour"
                lowerbound_x = Now.AddHours(-1).ToString(Format("dd/MM/yyyy hh:mm:ss"))
            Case "Month"
                lowerbound_x = Now.AddMonths(-1).ToString(Format("dd/MM/yyyy hh:mm:ss"))
            Case "Week"
                lowerbound_x = Now.AddDays(-1).ToString(Format("dd/MM/yyyy hh:mm:ss"))
            Case "Year"
                lowerbound_x = Now.AddYears(-1).ToString(Format("dd/MM/yyyy hh:mm:ss"))
            Case "Quater"
                lowerbound_x = Now.AddYears(-0.25).ToString(Format("dd/MM/yyyy hh:mm:ss"))
            Case "Day"
                lowerbound_x = Now.AddDays(-1).ToString(Format("dd/MM/yyyy hh:mm:ss"))
        End Select
        x_removeatrange = x.ToList
        y_removeatrange = y.ToList
        For j = 0 To x_removeatrange.Count - 1
            Try
                If x_removeatrange(j) < lowerbound_x Then
                    y_removeatrange.RemoveAt(j)
                    x_removeatrange.RemoveAt(j)
                End If
            Catch
            End Try
        Next
        ReDim x(x_removeatrange.Count - 1)
        x = x_removeatrange.ToArray()
        ReDim y(y_removeatrange.Count - 1)
        y = y_removeatrange.ToArray()
