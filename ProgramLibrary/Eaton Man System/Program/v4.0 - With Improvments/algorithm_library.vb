'import librarys used in module
...
Module algorithm_library
    'create settings instance and load them to be viewed on any form
    ...
    Public Sub loadsettings()
        ...
        FileClose(1)
    End Sub
    'function to see if password is active
    Function check_password()
        ...
        Else
            ...
            If password_record.active = True Then
                FileClose(1)
                Return True
            End If
        End If
        ...
        Return False
    End Function
    'convert from/to simple password encryption
    Public Function SimpleCrypt(ByVal Text As String) As String
        Dim strTempChar As String = String.Empty
        Dim i As Integer
        For i = 1 To Len(Text)
    ...
    'check to see if entered file location is valid
            ...
            If m2 <= high - low Then
                If m1 <= middle - low Then
                    If CInt(temp(m1, 0)) > CInt(temp(m2, 0)) Then
                        '3 rows to keep 2d array record integrity
    ...
    ''' </summary>
    ''' <param name="searchValue">ID of Product Record.</param>
    ''' <remarks></remarks>
    ...
    ''' </summary>
    ''' <param name="searchValue">ID of Component Record.</param>
    ''' <remarks></remarks>
    ...
    ''' <summary>
    ''' Returns Component Record Based On Search Value
    ''' </summary>
    ''' <param name="attribute">ID/CODE/DESC [Search Param]</param>
    ...
    ''' <summary>
    ''' Returns Location Record Based On Search ID
    ''' </summary>
    ''' <param name="searchValue">ID [Search Param]</param>
    ''' <remarks></remarks>
    ...
    ''' <summary>
    ''' Returns Job Record Based On Search ID
    ''' </summary>
    ''' <param name="searchValue">ID [Search Param]</param>
    ''' <remarks></remarks>
    ...
    ''' <summary>
    ''' Returns Product Component Record Based On Search IDs
    ''' </summary>
    ''' <param name="searchValueProduct">Product ID [Search Param]</param>
    ''' <param name="searchValueComp">Component ID [Search Param]</param>
    ''' <remarks></remarks>
    ...
    ''' <summary>
    ''' Returns Component Location Record Based On Search IDs
    ''' </summary>
    ''' <param name="searchValueLocation">Location ID [Search Param]</param>
    ''' <param name="searchValueComp">Component ID [Search Param]</param>
    ''' <remarks></remarks>
    ...
    ''' <summary>
    ''' Returns The Location Description Based On Component Entered
    ''' </summary>
    ''' <param name="compID">Component ID [Search Param]</param>
    ''' <remarks></remarks>
    Public Function getLocationFromComp(ByVal compID As Integer)
        FileOpen(7, "ComponentLocation.dat", OpenMode.Random, , , Len(component_location_record))
        For r = 1 To LOF(7) / Len(component_location_record)
            FileGet(7, component_location_record, r)
    ...
    ''' <summary>
    ''' Returns Location Record Based On Search Value and Search Type
    ''' </summary>
    ''' <param name="attribute">ID/CODE/DESC [Search Param]</param>
    ''' <param name="value">Value To Search By</param>
    ''' <remarks></remarks>
    ...
    'Sets Up Formatting of the Data Grid View 
    ...
    'Converts milliseconds to date time structure
    ...
    'clears record editor list view boxes and then populates them
    Public Sub populate_all()
        ...
        frmRecordEditor.lv_locations.Items.Clear()
    ...
    ''' <summary>
    ''' Populate list box of set file
    ''' </summary>
    ''' <param name="file">Enter File To Populate By</param>
    ''' <remarks></remarks>
    ...
    ''' <summary>
    ''' Saves Production Requirements to text file at given location
    ''' </summary>
    ''' <param name="FileLocation">Location To Print TXT File To</param>
    ''' <remarks></remarks>
    ...
    ''' <summary>
    ''' Saves Word Document Of Required Components
    ''' </summary>
    ''' <param name="FileLocation">Location To Print TXT File To</param>
    ''' <remarks></remarks>
    ...
    Word.WdParagraphAlignment.wdAlignParagraphRight
        ...
        wa.Quit()
    End Sub
    'does the same as word, then converts it to pdf
    ...
    'prints Excel Production requirements
    ...
    'debug output if closing office program fails
    ...
    'converts number to its alphabet position
    'eg a=1 b=2
    ...
    'disable and enable text field inputs as necassary
    Sub allowaccess(ByVal allow As String)
        ...
        End Select
    End Sub
    'check to see if product with a given id exists
    Public Function checkProductRecord(ByVal searchValue As String)
        ...
        Return False
    End Function
    'check to see if a component with a given id exists
    Public Function checkComponentRecord(ByVal searchValue As String)
        ...
        Return False
    End Function
    'check to see if a location with a given id exists
    Public Function checkLocationRecord(ByVal searchValue As String)
        ...
        Return False
    End Function
    'check to see if a component id and a location id share a record
    Public Function checkComponentLocationRecord(ByVal searchValueComp As String, ByVal searchValueLocation As String)
        ...
        End Try
    End Function
    'check to see if product id and component id share a record
    Public Function checkProductComponentRecord(ByVal searchValueProduct As String, ByVal searchValueComp As String)
        ...
        End Try
    End Function
    'get record number of where product id and component id share a record
    Public Function getProductComponentRecordNumber(ByVal searchValueProduct As String, ByVal searchValueComp As String)
        ...
        Return -1
    End Function
    'get record number of where location id and component id share a record
    ...
    'cleans set file by ensuring all records exist and no relationships are duplicated
        ...
        Select Case file
            Case "Products"
                ...
                For i = 1 To (LOF(4) / Len(product_record))
                    ...
                    If product_record.product_id <= 0 Or Trim(product_record.product_code) = String.Empty Or Trim(product_record.product_desc) = String.Empty Then
    ...
    'adds production log entry for the given id and quantity to either comp or product, for later analysis
        ...
        If compOrProduct = "Component" Then
            For i = 0 To (arrayToAdd.Length / 2) - 1
                If arrayToAdd(i, 0) <> String.Empty Then
                    ...
                    component_trend_record.dateOutput = Now.ToString(Format("dd/MM/yyyy hh:mm:ss tt"))
        ...
        Else
            For i = 0 To (arrayToAdd.Length / 2) - 1
                If arrayToAdd(i, 0) <> String.Empty Then
                    ...
                    product_trend_record.dateOutput = Now.ToString(Format("dd/MM/yyyy hh:mm:ss tt"))
    ...
    End Sub
    Sub check_update()
        Dim currentVersion As String = "v1.0"
        If update_required(currentVersion) Then
            Dim result As Integer = MessageBox.Show("Update Required, Would You Like To Update Now?", "EMS - Update Alert", MessageBoxButtons.YesNo)
            If result = DialogResult.Yes Then
                Process.Start(CurDir() & "\EMS-UpdateClient.exe")
                End
            End If
        Else
            Dim fecha As Date = IO.File.GetCreationTime(Assembly.GetExecutingAssembly().Location)
            MsgBox("System Version: " & currentVersion & vbNewLine & "System Up To Date. " & fecha.ToShortDateString().ToString)
        End If
    End Sub
    Function update_required(ByVal currentVersion As String)
        Dim recentVersion As String = currentVersion
        Try
            recentVersion = New System.Net.WebClient().DownloadString("http://82.19.253.90/eatonmanufacturingsystem/update")
        Catch
        End Try
