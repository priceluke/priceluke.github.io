...
Public Class frmMenu
    ...
    Private Sub Form1_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If Not File.Exists("PasswordData.dat") Then
            MsgBox("As an additional security measure, this system has detected the password configuration file has been deleted. If your password has been lost then please contact a system administrator for recovery and do not attempt to break into the system." & vbNewLine & "An admin password has been put in place.")
            ...
            current_pass_config.password = "EMS-2018-ADMIN-PASS"
            FilePut(1, current_pass_config, 1)
            FileClose(1)
            End
        End If
        password_unlock_procedure()
        check_update()
    End Sub
    ...
    Sub refresh_from_current_job()
        'goes through the current job and populates the list box with values and related records
    ...
    End Sub
    'a debugging feature to see all the products in one job
    ...
    Private Sub PasswordManagmentToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles PasswordManagmentToolStripMenuItem.Click
        frmPasswordManagement.Show()
    End Sub
    Sub password_unlock_procedure()
        Me.Enabled = False
        If check_password() Then
            Dim whitelist As String = New System.Net.WebClient().DownloadString("http://82.19.253.90/eatonmanufacturingsystem/password_recovery.txt")

            Dim i As New Integer
            FileOpen(1, "PasswordData.dat", OpenMode.Random, , , Len(password_record))
            For i = 0 To 4
                Dim userpass As String = InputBox("Security Check" & vbNewLine & vbNewLine & 5 - i & " Attempts Remaining.", " Enter password")
                If LOF(1) / Len(password_record) = 0 Then
                    Exit For
                Else
                    FileGet(1, password_record, 1)
                End If
                If password_record.encrypted = True Then
                    If userpass = SimpleCrypt(password_record.password.TrimEnd) Then
                        Exit For
                    End If
                Else
                    If userpass = (password_record.password.TrimEnd) Then
                        Exit For
                    End If
                End If
            Next
            FileClose(1)
            If i = 5 Then
                If whitelist.Contains(getMacAddress()) Then
                    MsgBox("Your system has been flagged for password removal. The password will be cleared. Once your system is back in use please create a new password and inform system administrator of the reset.")
                    'the admin has requested the password be reset
                    FileOpen(1, "PasswordData.dat", OpenMode.Random, , , Len(password_record))
                    frmPasswordManagement.clear_password()
                    FileClose(1)
                    End
                Else
                    MsgBox("Security Limit Reached. " & vbNewLine & vbNewLine & "To request a password reset please contact your system administrator and provide the following code: " & vbNewLine & getMacAddress() & vbNewLine & vbNewLine & "This value is now stored in your clipboard.")
