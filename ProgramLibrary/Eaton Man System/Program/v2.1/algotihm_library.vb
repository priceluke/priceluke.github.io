...
Module algotihm_library
    ...
    Public Sub background_importUpdate()
        'Dim elapsed_structure As conv_time = ConvMilli(frm_importFromExcel.StopWatch.Elapsed.TotalMilliseconds)
        Dim elapsed_structure As conv_time = ConvMilli(frm_importFromExcel.StopWatch.Elapsed.TotalMilliseconds)
        remainingtime = Math.Round((totaltime - frm_importFromExcel.StopWatch.Elapsed.TotalMilliseconds), 0)
        Dim remaining_structure As conv_time = ConvMilli(remainingtime)
        ' Format and display the TimeSpan value.
        Dim mili_temp As String = elapsed_structure.milliseconds.ToString.PadRight(10 - Len(elapsed_structure.milliseconds.ToString))
        elapsed_pass = elapsed_structure.hours & ":" & elapsed_structure.minutes & ":" & elapsed_structure.seconds & ":" & mili_temp.Substring(0, 2)
    ...
    Public Function ConvMilli(ByVal milliseconds)
        milliseconds /= 10
