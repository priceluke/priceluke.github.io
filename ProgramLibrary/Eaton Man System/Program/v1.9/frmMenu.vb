...
Public Class frmMenu
        ...
        Else
            For i = 1 To LOF(filenum) / Len(product_record)
                ...
                If e = "Description" Then
                    ...
                    End If
                ElseIf e = "Code" Then
                    If product_record.product_code = search_parameter Then
                        combo_productID.Text = product_record.product_id
    ...
    Private Sub btnConfirmJob_Click(sender As Object, e As EventArgs) Handles btnConfirmJob.Click
        ...
        Else
            ...
            For item = 0 To listofproducts.Count - 1
            Next
                ...
                For i = 1 To LOF(1) / Len(product_component_record)
                    ...
                    For d = 0 To listofproducts.Count - 1
                        If product_component_record.product_id = listofproducts(d) Then
                            ...
                            For t = 0 To Print_Components.Rows.Count - 1
                                If Print_Components.Rows(t).Cells("ID").Value = (product_component_record.component_id) Then
                                    Print_Components.Rows(t).Cells("Quantity").Value += (listofproducts(d) * product_component_record.quantity)
                                    Record_Present = True
                                End If
                            Next
                            If Record_Present = False Then
                                Print_Components.Rows.Add(product_component_record.component_id, Component_Name_Temp, listofproducts_Quantity(d) * product_component_record.quantity)
                            End If
                        End If
                    Next
                Next
    ...
    Public Sub update_menu()
        ...
        Else
            For i = 1 To LOF(filenum) / Len(product_record)
                ...
                list_productcode.Add(product_record.product_code)
            Next i
        End If
        FileClose(filenum)
        '''''''''''''''''''''''''''''''''''''''''''
    ...
    Sub refresh_from_current_job()
        ...
        For m = 0 To (running_quantity.Length / 4) - 1
            ...
            Else
                list_currentJob.Items.Add(running_quantity(m, 0) & ": " & running_quantity(m, 2) & ": " & running_quantity(m, 1) & ": " & running_quantity(m, 3))
                Dim filenum = FreeFile()
